...
## 10. Pre-rendering requirement (full builds only)

The niche template is a Vite SPA. Google renders JS pages, but it takes days longer
than pre-rendered HTML. For full production builds, Stage 10.1 (build agent) must
enable pre-rendering before deploying.

Pre-rendering approach: `@prerenderer/plugin-vite` + `@prerenderer/renderer-puppeteer`
(puppeteer is already in `devDependencies`). The build agent adds this to
`vite.config.js`, supplies the full route list from `sitemap.json`, then runs
`vite build`. Each route gets its own pre-rendered `index.html` in `dist/`.

This has zero impact on the mockup dev server (`npm run dev`) — pre-rendering only
activates during `vite build`.

Do NOT deploy to Vercel without pre-rendering. A JS-only deploy means Googlebot
reads `<div id="root"></div>` and ranks the site for nothing.

---

## 11. GEO — Generative Engine Optimization (AI Search)

AI engines (ChatGPT, Perplexity, Google AI Overviews, Bing Copilot, Gemini) surface
local business content in response to "best roofer near me" style queries. Optimize
for AI citation in addition to Google SERP. These steps run at Stage 10.2.

### 11.1 AI crawler access in robots.txt

The generated `robots.txt` must include explicit Allow rules for every major AI
crawler. Never disallow any AI crawler — local businesses gain more from AI citation
than they lose from potential training data use.
...
Generate `dist/llms.txt` at build time. This file tells AI crawlers what the site
contains and which pages to prioritize for citation.

```
# {Business Name}
> {One-sentence plain-English description of the business and location.}

## Services
- [{Service Name}](https://{domain}/services/{slug}/)
  (one line per service)

## Service Areas
- [{City, State}](https://{domain}/areas/{city-slug}/)
  (one line per service area from serviceAreas[])

## About
{Business Name} is a licensed {niche type} serving {primary city} since {year}.
{review_count} verified reviews at {rating} stars on Google.
Contact: {phone}
```

Populate from `brand-dna.json` + `sitemap.json` + `research-data.json`. Place the
file at domain root (`dist/llms.txt`), same level as `robots.txt`.

### 11.3 Speakable schema

Mark the hero headline and the first trust bar sentence as `speakable` so AI
engines know which text to quote when citing the business.

Add to the homepage's LocalBusiness JSON-LD:
```json
"speakable": {
  "@type": "SpeakableSpecification",
  "cssSelector": ["h1", ".trust-bar-lead"]
}
```

Add CSS class `trust-bar-lead` to the first `<p>` tag inside TrustBar.jsx.

### 11.4 QAPage schema on service pages

As of May 2026 Google retired FAQPage rich results for all sites. Keep FAQPage
schema for its AI/LLM citation signal, but also add QAPage schema per question.
AI engines weight QAPage more heavily as verified Q&A.
...
```json
{
  ...
  "mainEntity": {
    "@type": "Question",
    "name": "How much does roof replacement cost in {city}?",
    "acceptedAnswer": {
      "@type": "Answer",
      "text": "Roof replacement in {city} typically costs $8,000–$18,000 ..."
    }
  }
}
```

Generate one QAPage block per FAQ question. Inject alongside the existing FAQPage
schema, not instead of it.

### 11.5 sameAs entity links

Populate the Organization and LocalBusiness schema `sameAs` arrays from
`research-data.json`. AI engines cross-reference these to build entity confidence.
...
Skip any platform not found in research. Never invent URLs.

### 11.6 Named entity reinforcement

Every page's first 100 words must contain a sentence naming the business, its
city, and its primary service together. This anchors the entity for AI training.

Example: "Apex Roofing has replaced and repaired roofs in Nashville since 2011."

One natural sentence. Never keyword-stuff.
