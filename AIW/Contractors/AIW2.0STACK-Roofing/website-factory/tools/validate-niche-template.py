...
def gate_1_brand_dna_shape(niche_dir: Path) -> tuple[bool, str]:
    ...
    # JS built-in array/string/object methods are never brand-dna shape keys.
    _JS_BUILTINS = {
        "length", "map", "filter", "find", "findIndex", "forEach", "reduce",
        "slice", "splice", "concat", "includes", "some", "every", "flat",
        "flatMap", "indexOf", "join", "push", "pop", "shift", "unshift",
        "sort", "reverse", "fill", "at", "keys", "values", "entries",
    ...
    for jsx_path in src_dir.rglob("*.jsx"):
        for access in _extract_jsx_brand_dna_accesses(jsx_path):
            ...
            keys = access.split(".")[1:]
            for key in keys:
                if key in _JS_BUILTINS:
                    break  # method call, not a shape access -- skip
...
def gate_2_jsx_parse(niche_dir: Path) -> tuple[bool, str]:
    """Syntactic sanity check on every .jsx file.

    Layer A (authoritative): Run ESLint with the project's own config. ESLint
    parses JSX correctly via espree and is the canonical parse check.

    Layer B (heuristic, WARN only): Balanced curly braces + balanced JSX tag
    counting. This heuristic cannot handle JSX attribute expressions or
    multi-line tags correctly, so failures here are treated as warnings if
    ESLint passes. Layer B findings are logged but do not fail the gate.

    Sentinels (`__REQUIRED__SOMETHING__`) are EXPECTED in the niche
    template at Module 2D time. Stage 10.1 fills them per client.
    """
    src_dir = niche_dir / "src"
    if not src_dir.exists():
        return False, f"src/ missing at {src_dir.relative_to(REPO_ROOT)}"

    # --- Layer A: ESLint (authoritative) ---
    eslint_bin = niche_dir / "node_modules" / ".bin" / "eslint"
    eslint_bin_cmd = niche_dir / "node_modules" / ".bin" / "eslint.cmd"
    if eslint_bin_cmd.exists():
        eslint_cmd = ["cmd", "/c", str(eslint_bin_cmd), "src/"]
    elif eslint_bin.exists():
        eslint_cmd = [str(eslint_bin), "src/"]
    else:
        eslint_cmd = None
    if eslint_cmd:
        result = subprocess.run(
        ...
        if result.returncode != 0:
            return False, f"ESLint found errors (authoritative parse):\n{result.stdout}\n{result.stderr}"
        eslint_note = "ESLint: OK. "
    else:
        eslint_note = "ESLint not found (no node_modules yet); heuristic checks only. "

    # --- Layer B: heuristic brace + tag balance (WARN only) ---
    heuristic_warns: list[str] = []
    jsx_count = 0
    for path in src_dir.rglob("*.jsx"):
        ...
        closes = clean.count("}")
        if opens != closes:
            heuristic_warns.append(
                f"  WARN {path.relative_to(REPO_ROOT)}: brace imbalance "
                f"({opens} open vs {closes} close after strip) -- may be heuristic FP"
            )
            continue
        ...
        if unbalanced_tags:
            ...
            heuristic_warns.append(
                f"  WARN {path.relative_to(REPO_ROOT)}: tag imbalance ({tag_detail}) -- may be heuristic FP"
            )

    warn_note = ""
    if heuristic_warns:
        warn_note = f" Heuristic warns (non-blocking -- ESLint is authoritative):\n" + "\n".join(heuristic_warns)

    return True, f"{eslint_note}parse-tier checks on {jsx_count} JSX files.{warn_note}"
...
def gate_3_vite_build(niche_dir: Path, skip_install: bool) -> tuple[bool, str]:
    ...
    # On Windows, npm is npm.cmd. Use shell=True to let the OS resolve it.
    _npm = "npm"
    _run_kw = {"cwd": niche_dir, "capture_output": True, "text": True, "shell": True}

    try:
        if not skip_install:
            install = subprocess.run(f"{_npm} install --silent", **_run_kw)
            if install.returncode != 0:
                return False, f"npm install exit={install.returncode}\n\nSTDOUT:\n{install.stdout[-2000:]}\n\nSTDERR:\n{install.stderr[-2000:]}"

        build = subprocess.run(f"{_npm} run build", **_run_kw)
...
def _write_synthetic_filled(brand_dna_js: Path, brand_dna_example: Path) -> tuple[bool, str]:
    ...
    try:
        # Run the helper as a top-level module script (not via dynamic import)
        # so that process.stdout.write() at module top-level is captured.
        result = subprocess.run(
