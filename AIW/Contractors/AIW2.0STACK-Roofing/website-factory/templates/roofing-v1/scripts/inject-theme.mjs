...
async function loadBrandDNA() {
  ...
  const url = BRAND_DNA.startsWith("file://") ? BRAND_DNA : new URL(`file:///${BRAND_DNA.replace(/\\/g, "/")}`).href;
...
function buildFontImports({ headingFontUrl, bodyFontUrl, accentFontUrl }) {
  const heading = normaliseGoogleFontUrl(headingFontUrl);
  const body = normaliseGoogleFontUrl(bodyFontUrl);
  const accent = normaliseGoogleFontUrl(accentFontUrl);
  const seen = new Set();
  const lines = [];
  for (const url of [heading, body, accent]) {
    if (url && !seen.has(url)) { seen.add(url); lines.push(`@import url('${url}');`); }
  }
  return lines.join("\n") + "\n";
}
