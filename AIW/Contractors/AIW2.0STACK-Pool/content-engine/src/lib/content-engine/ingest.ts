...
async function fetchAndExtract(url: string): Promise<{
  ...
  try {
    ...
    // Lazy-load jsdom to avoid loading the module (and its ESM-only
    // transitive deps) on every serverless cold start. Text, YouTube,
    // and social-reel ingestion paths never reach this function.
