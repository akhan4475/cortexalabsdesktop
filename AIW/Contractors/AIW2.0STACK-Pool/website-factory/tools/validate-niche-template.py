...
def gate_1_brand_dna_shape(niche_dir: Path) -> tuple[bool, str]:
    ...
    # Standard JS array/object/string methods that are valid terminal
    # accessors on brandDNA fields and should NOT be checked against the
    # canonical shape.
    _JS_BUILTINS: set[str] = {
        "length", "map", "filter", "find", "findIndex", "forEach", "reduce",
        "reduceRight", "some", "every", "includes", "slice", "splice", "join",
        "concat", "push", "pop", "shift", "unshift", "sort", "reverse", "flat",
        "flatMap", "keys", "values", "entries", "indexOf", "lastIndexOf",
        "toString", "toLocaleString", "hasOwnProperty", "then", "catch",
        "split", "trim", "toLowerCase", "toUpperCase", "replace", "match",
        "charAt", "charCodeAt", "substring", "startsWith", "endsWith",
    }
        ...
        for access in _extract_jsx_brand_dna_accesses(jsx_path):
            ...
            for key in keys:
                if key in _JS_BUILTINS:
                    break  # valid JS method/property, stop checking this chain
...
def _count_jsx_tag_balance(text: str) -> dict[str, int]:
    ...
    delta: dict[str, int] = {}
    # Tag name pattern: identifier optionally followed by dot-notation segments
    # (e.g. motion.div, AnimatePresence, etc.)
    _TAG_PAT = r"[A-Za-z][A-Za-z0-9]*(?:\.[A-Za-z][A-Za-z0-9]*)*"
    # Opening tags: <Component or <motion.div — exclude self-closing (/>)
    for m in re.finditer(r"<(" + _TAG_PAT + r")\b[^>]*?(?<!/)>", clean):
        delta[m.group(1)] = delta.get(m.group(1), 0) + 1
    # Closing tags: </Component> or </motion.div>
    for m in re.finditer(r"</(" + _TAG_PAT + r")\s*>", clean):
        delta[m.group(1)] = delta.get(m.group(1), 0) - 1
...
def _write_synthetic_filled(brand_dna_js: Path, brand_dna_example: Path) -> tuple[bool, str]:
    ...
    try:
        result = subprocess.run(
            ["node", "--input-type=module", "-e", f"import('{helper_path.as_uri()}').catch(e => {{ console.error(e); process.exit(1); }})"],
